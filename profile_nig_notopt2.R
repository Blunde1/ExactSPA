...
setwd("C:/Users/Berent/Projects/it-ift/implementation/plotting/test_plots")
pdf("nig_spafail_iftfail.pdf", width=7, height=4+1/3)
plot(gamma.val3, nll_espa, type="o",pch=3, lwd=4,
     #ylim=c(0,30000),
     ylim=c(min(nll_spa,nll_espa), max(nll_espa,nll_spa)),
     ylab="Negative log-likelihood", xlab=expression(gamma))
lines(gamma.val3, nll_bessel, type="b",col="red", pch=2, lwd=2)
lines(gamma.val3, nll_spa, lty=2, col="blue", lwd=2)
points(gamma.val3, nll_simpson, pch=4, col="green", lwd=2)
#lines(gamma.val3, nll_respa, lty=5, col="green", lwd=2)
legend("topleft", c("Saddlepoint adjusted IFT", "Bessel implementation", "Saddlepoint approximation", "Simpson IFT"),
       col=c("black","red", "blue","green"), pch=c(3,2,NA,4), lwd=c(4,2,2,2), lty=c(1,1,2,NA))
dev.off()
