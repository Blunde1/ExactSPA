...
param <- c("r"=0.08,"sigma"=0.1,"lambda"=100,"mu"=-0.001,"nu"=0.015, "x0"=0, "t"=1/250)
...
s <- seq(-1000,1000, length.out=200)
y <- sapply(s, recf)
plot(s,y)
...
plot(u, y, xaxt="n", yaxt="n", ylim=c(min(y), max(y)),
     , main="MJD: x0=0, x_t=0", type="l", col="1", lwd=2, lty=2,
...
legend("topright", legend=c("Real part of standardised CF", "Standard normal"), col=c("black", "red"), lty=c(2,2))
...
mjd.e <- mjd.cgf.1(0)
mjd.v <- mjd.cgf.2(0)
x <- seq(mjd.e - 9*sqrt(mjd.v), mjd.e+5*sqrt(mjd.v), length.out = 200)
p.0.x <- sapply(x, p.0)
plot(x, p.0.x,type="l")
