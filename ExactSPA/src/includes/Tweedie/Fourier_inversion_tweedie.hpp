...
#ifndef __FOURIER_INVERSION_TWEEDIE_HPP_INCLUDED__
#define __FOURIER_INVERSION_TWEEDIE_HPP_INCLUDED__

#include "../complex.hpp"
#include "saddlepoint_approximation_tweedie.hpp"


/** Simpson's rule for IFT
 */
template<typename T>
T ift_simpson0(lcf_stand_tweedie<T> lcf_stand, double length, int n){
...
template<typename T>
T ift_simpson(cgf_tweedie<T> cgf, T x, double length, int n){
    T simpson_integral = 0.0;
    T a = 0.0, b = length;
    T h = (b-a) / n;
    cType<T> i(0,1);
...
#endif //__FOURIER_INVERSION_NCHISQ_HPP_INCLUDED__
