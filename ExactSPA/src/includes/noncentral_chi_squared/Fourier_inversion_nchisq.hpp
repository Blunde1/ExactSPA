...
#ifndef __FOURIER_INVERSION_NCHISQ_HPP_INCLUDED__
#define __FOURIER_INVERSION_NCHISQ_HPP_INCLUDED__
 ...
 */
template<typename T>
T ift_simpson0(lcf_stand_nchisq<T> lcf_stand, double length, int n){
...
template<typename T>
T ift_simpson(cgf_nchisq<T> cgf, T x, double length, int n){
    T simpson_integral = 0.0;
    T a = 0.0, b = length;
    T h = (b-a) / n;
    cType<T> i(0,1);
