...
mjd.e <- mjd.cgf.1(0)
mjd.v <- mjd.cgf.2(0)
x <- seq(mjd.e - 6*sqrt(mjd.v), mjd.e+2*sqrt(mjd.v), length.out = 200)
p.0.x <- sapply(x, p.0)
plot(x, p.0.x,type="l")
...
setwd("C:/Users/Berent/Projects/Berent/sp_adjusted_inversion/Figures")
pdf("mjd_prob_x_v2.pdf", width=7, height=4+1/3)
par(mar=c(5,6, 3, 6))
plot(x, spa1, xaxt="n", yaxt="n", ylim=c(0, max(spa1*p.0.x*sqrt(2*pi))),
     main="", type="l", col="1", lwd=2, lty=2,
     ylab=expression(paste("density approximation")),
...
axis(1)
legend("topleft", legend=c("Orindary spa", "Exact"), col=c("black", "red"),
       lty=c(2,2), lwd=c(2,2))
# par(new=T)
